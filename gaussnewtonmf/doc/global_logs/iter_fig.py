...
for data in datas:
    ...
    for method in methods:
        ...
        ant_x = [x for x in ant_x[:upper_idx]]
        ...
        line = mlines.Line2D([], [], ls='-', color=color_map[method], markersize=8, marker=marker_map[1][0],
                             label=method, linewidth=2.0, markeredgewidth=0.0)
        line2ds.append(line)
    ...
    plt.savefig("{0}.obj.iter.eps".format(data), format='eps', dpi=1000)
    plt.show()
    plt.clf()
...
for data in datas:
    ...
    for method in methods:
        ...
        ant_x=[x for x in ant_x [idy:idx]]
        ant_y=ant_y[idy:idx]
        line = mlines.Line2D([], [], ls='-', color=color_map[method], markersize=8, marker=marker_map[1][0],
                             label=method, linewidth=2.0, markeredgewidth=0.0)
        line2ds.append(line)
    ...
    plt.legend(handles=line2ds, fontsize=12)
    plt.savefig("{0}.rmse.iter.eps".format(data,), format='eps', dpi=1000)
    plt.show()
    plt.clf()
